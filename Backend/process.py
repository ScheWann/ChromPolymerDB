...
"""
Download GSE CSV for a specific cell line, resolution, and cell ID
File naming convention under ./GSE: {cell_line}/{resolution}/{cell_id}.csv
"""
def download_gse_csv(cell_line: str, resolution: str, cell_id: str):
    # Construct the file path based on the expected structure
    file_path = os.path.join("./GSE", cell_line, resolution, f"{cell_id}.csv")
    ...
    return send_file(
        file_path,
        as_attachment=True,
        download_name=f"{cell_line}_{resolution}_{cell_id}.csv",
        mimetype='text/csv'
    )
...
"""
Return currently existing GSE cell ID options in the given cell line and resolution
"""
def get_gse_cell_id_options(cell_line: str, resolution: str):
    with db_conn() as conn:
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute(
                """
                ...
                FROM gse
                WHERE cell_line = %s AND resolution = %s
                """,
                (cell_line, resolution)
            )
